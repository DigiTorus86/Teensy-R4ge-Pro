...
TFT_group_op12::TFT_group_op12():TFT_group()
{
    _items[0] = new TFT_select_wave(0, 36, "OSC1");
    ...
    _items[12] = &env1; // new TFT_envelope(260, 36, "OP1");

    _items[13] = new TFT_select_wave(0, 122, "OSC2");
    ...
    _items[25] = &env2; //new TFT_envelope(260, 122, "OP2");
    _items[26] = new TFT_checkbox(260, 190, "Fixed");

    // Set custom ranges and starting values for controls
    _items[0]->setValue(SYN_WAVE_SINE); // Osc1
    _items[1]->setValue(1); // Op1 Coarse=1
    _items[4]->setValue(100); // Op1 Level
    _items[5]->setValue(100); // Op1 Atk lvl
    _items[6]->setValue(60); // Op1 Dec lvl
    _items[7]->setValue(60); // Op1 Sus lvl

    _items[9]->setValue(5); // Op1 Atk dur
    _items[10]->setValue(10); // Op1 Dec dur
    //_items[10]->setValue(100); // Op1 Sus dur
    _items[11]->setValue(70); // Op1 Rel dur

    _items[14]->setValue(1); // Op2 Coarse=1
    _items[17]->setValue(100); // Op2 Osc lvl

    _item_count = 27;
    _items[0]->setSelected(true);
}

void TFT_group_op12::updateEnvelopes()
{
    if (getChanged())
    {
        _env_cfg = {
        ...
        env2.setConfig(&_env_cfg);
    }
}

void TFT_group_op12::getOpConfig(uint8_t op_num, SYN_op_config_t *op_cfg)
{
    if (op_num == 1)
    {
...
void TFT_group_op12::setOpConfig(uint8_t op_num, SYN_op_config_t *op_cfg)
{
    if (op_num == 1)
    {
